...
Chrome extension (Vite + React TSX + CRXJS). One-click: meta.ai image → flow.google.com video (supports project URLs like flow.google.com/project/[projectid]).

## Dev
1. `npm install`
2. `npm run dev` (popup HMR) or `npm run build` → `dist/`
3. `chrome://extensions` → Developer mode → Load unpacked → select `dist/`
4. Log in to https://www.meta.ai and https://flow.google.com first.
5. Open popup → Start pipeline. Keep both tabs open.

## Settings
Model: omni 1.1 flash (default), veo 3.1 fast/lite/quality. Time: 4s/6s/8s/10s. Aspect: 16:9/9:16/1:1. Size: 720p/1080p.
